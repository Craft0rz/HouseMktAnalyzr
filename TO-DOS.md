...
## Show Data Source Transparency - 2026-02-07 23:31

- **Add "estimated" indicator when using CMHC averages instead of actual data** - Display small text label clarifying when rental income, rent estimates, or other metrics come from CMHC averages vs actual Centris-scraped data (e.g. declared gross revenue). **Problem:** Users see a rental income figure (e.g. $1,900/mo) without knowing it's a CMHC zone average rather than the listing's actual declared revenue ($84K/yr = $7,000/mo). This is misleading and erodes trust in the analysis. **Files:** `frontend/src/components/PropertyDetail.tsx`, `frontend/src/components/PropertyTable.tsx`, `src/housemktanalyzr/models/property.py:58-60` (gross_revenue field), `src/housemktanalyzr/analysis/calculator.py:312-341` (estimate_rent_from_listing priority logic), `backend/app/routers/analysis.py`. **Solution:** Add a `rent_source` field to InvestmentMetrics (e.g. "declared", "cmhc_estimate", "enrichment") so the frontend can show a subtle label like "Est. (CMHC avg)" next to the rent figure when it's not from actual listing data.
